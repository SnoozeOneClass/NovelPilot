...
- 固定在项目目录启动，已命名作品存放在 `books/<书名>/`。新书直接共创，书名可选，不能强制启动时先填写；内容/大纲方向形成后再由用户确定或模型生成书名。未命名工作区由程序管理，不让目录规则阻碍创作。
- 前端采用完整 TUI 工作台，参考用户提供的 ainovel-cli 多栏布局；常用操作通过按钮和可选择菜单完成，不能仅提供命令输入区。统计显示真实数据，未知值不编造。
- 按阶段设计界面：共创用对话/完整草稿双栏，写作后才显示监控三栏。模型设置单页原位编辑、统一保存；操作菜单按阶段筛选并支持搜索，避免连续问答式配置和大量空监控面板。
