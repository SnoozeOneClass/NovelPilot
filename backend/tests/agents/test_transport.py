    ...
    async def exercise() -> ActivationRequestBudget:
        ...
        async with httpx.AsyncClient(transport=transport) as client:
            with pytest.raises(httpx.ReadTimeout):
                async with client.stream("POST", "https://provider.example/v1/responses") as response:
...
def test_output_repair_and_full_task_replay_share_the_six_request_oracle() -> None:
