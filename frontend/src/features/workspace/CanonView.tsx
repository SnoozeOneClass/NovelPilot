...
import styles from "./CanonView.module.css";
  ...
  return (
    <section className={styles.view}>
      <header className={styles.heading}>
        ...
        <button className={styles.iconButton} title="刷新正史状态" disabled={refreshing} onClick={() => void refresh()}>
          <RefreshCw size={17} />
        </button>
      </header>
      ...
      <nav className={styles.tabs}>
        {tabs.map((tab) => (
          <button key={tab.id} className={activeKind === tab.id ? styles.active : ""} onClick={() => setActiveKind(tab.id)}>
            {tab.label}
      ...
      <div className={styles.toolbar}>
        ...
        <button className={styles.outlineButton} onClick={() => setShowRaw((value) => !value)}>
      ...
      <div className={styles.contentGrid}>
        <section className={styles.items}>
          {entries.map(([id, value]) => {
            ...
            return (
              <article key={id}>
                ...
                <button className={styles.textLink} onClick={() => setShowRaw(true)}>
          ...
          {entries.length === 0 && (
            <div className={styles.empty}>
              <div className={styles.avatar}>{initials(activeTab.singular)}</div>
              <h2>还没有已提交的{activeTab.singular}</h2>
              <p>章节通过验证并提交状态补丁后，相关条目会出现在这里。</p>
            </div>
          )}
          <footer className={styles.sourceLine}>
            <button className={styles.textLink} onClick={() => onSelectArtifact(canonFiles[activeKind])}>查看数据文件</button>
        ...
        <aside className={styles.summaryPanel}>
          ...
          <section className={styles.recentCommits}>
            <h3>最近提交</h3>
            <ul>
              ...
              {recentCommits.length === 0 && <li className={styles.emptyLine}>暂无状态提交</li>}
            </ul>
          </section>
        </aside>
      </div>
      ...
      {showRaw && (
        <section className={styles.rawPanel}>
          <header>
            ...
            <button className={styles.textLink} onClick={() => setShowRaw(false)}>关闭</button>
          </header>
