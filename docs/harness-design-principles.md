...
状态转移：harness 根据已提交事实和验证结果决定在当前正式 Arc baseline 的计划检查点前继续弧内章节；章节数到达检查点时，只触发最低限度的 Arc 收束评估。只有 Arc 契约的收束条件被正式事实满足后才能进入下一弧；如果仍需生成新的故事事实，必须由 Arc 层审阅后通过正式 Arc 修订冻结新的计划和检查点，不能在原 baseline 下静默追加。其他合法出口包括逐级升级到 Book Loop、要求下层证据纠正、等待用户或失败暂停。
